const ErrorMessages = {
	VALIDATION: {
		...
		INVALID_DATA_TYPE: (value?: string, expectedType?: string) => `${value} value must be of type ${expectedType}`,
	...
	AUTH: {
		...
		USERNAME_LENGTH_MAX: (min: number, max: number) => `username must be greater than ${min} character and less than ${max + 1} characters.`,
		NAME_LENGTH_MAX: (min: number, max: number) => `Full name must be greater than ${min} character and less than ${max} characters.`,
